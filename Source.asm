...
.data
	...
	path_length dword 0
	info1 byte "Name: Syeda Tayyiba Fatima        NU Id: 23k-0853",10,13,0
	info2 byte "Name: Rehma Khurram        NU Id: 23k-0049",10,13,0
	info3 byte "--------------- << MAZE SOLVER >> ---------------",10,13,0
	boardMsg1 byte "This is the representation of Board in Memory:",10,13,0
	sl byte "----------------------------------",10,13,0
	boardMsg2 byte "Dump of offset ",0
	space byte "  ",0
	pathMsg1  byte "The path from S to E is :",0
	pathMsg2  byte "The path length is :",0
	pathMsg3  byte "Calling chek_path, returns:",0
	row word 0
	column word  0
	index word 0
	output byte 0
...
;-------------------------
;This function print my personal information
printInfo PROC
...
calculateBoardLength PROC
	...
	ret                                   ; length of borad is returned in ecx
...
checkpath:
	...
	check_down_step:
		inc row
		movsx ebx,row ; ebx used just to compare 
